...
Add one with `pnpm changeset`. `privatePackages` is off, so `@frictionsets/app` is never versioned or
published from here.

Not to be confused with `.agents/frictionsets`, which records friction rather than releases. The two
