...
app.composerAction('/', (c) => {
  const { actionData } = c
  const { castId, fid, messageHash, network, timestamp, url, state: { cast: { text } } } = actionData // [!code focus]
  return c.res({/* ... */})
}, {/**/})
```
...
- **Type**: `(response: ComposerActionResponse) => ComposerActionResponse`
