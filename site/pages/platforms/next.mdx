...
path of the website on Warpcast, they will be able to see the frame. This leverages the [`generateMetadata`](https://nextjs.org/docs/app/api-reference/functions/generate-metadata#generatemetadata-function) function built-in to Next.js

```tsx twoslash [app/page.tsx]
// @noErrors
import { extractFrameMetaTags } from 'frog'
import { type Metadata } from 'next'
// The base URL of your app
import { APP_URL } from './app-url'

export async function generateMetadata(): Promise<Metadata> {
  const frameTags = await extractFrameMetaTags(`${APP_URL}/api`)
  return {
    other: frameTags,
  }
}
