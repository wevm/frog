...
:::note
Note that if you have any suspended component within the frame co-located Next.js page, the request
for a frame from Warpcast will fallback for streaming, and although [`generateMetadata`](https://nextjs.org/docs/app/api-reference/functions/generate-metadata#generatemetadata-function)
makes sure `<head>` tags are [present within the first stream response](https://nextjs.org/docs/app/building-your-application/routing/loading-ui-and-streaming#seo), Warpcast currently
doesn't handle streams.

The consequence of such is timing out on a frame that is co-located with suspended page.

Since Next.js engine selectively decides wether to respond in a stream or not, we can selectively not render suspended components
with `isFrameRequest` util.
